...
const ImageUpload = ({ onChange }: ImageUploadProps) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            <label
                htmlFor="photo"
                className={`w-full block p-4 sm:p-6 md:p-8 lg:p-12 xl:p-16 border border-dashed rounded-sm sm:rounded-md md:rounded-lg lg:rounded-xl xl:rounded-2xl bg-[#FAF7F2] transition-colors ${error ? "border-[#E24B4A]" : "border-[#737373]"
                    }`}
            >
                <div className='flex flex-col items-center'>
                    ...
                    <div className='flex flex-col items-center mb-6 sm:mb-8 md:mb-10 lg:mb-12 gap-1.5 sm:gap-2 md:gap-2.5 lg:gap-3'>
                        <h4 className='text-black text-sm sm:text-base md:text-lg lg:text-xl text-center'>
                            Upload your business document
                        </h4>
                        <p className='text-[#525252] text-[10px] sm:text-xs md:text-sm lg:text-base text-center'>
                            Drag and drop your business registration document here or choose a file below
                        </p>
                    </div>
            ...
            {/* Error message */}
            {error && (
                <p className="text-[#E24B4A] text-[10px] md:text-xs lg:text-sm font-semibold">
                    {error}
                </p>
            )}
        </div>
